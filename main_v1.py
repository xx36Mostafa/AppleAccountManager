...
class apple:
    ...
    def preparing(self):
        ...
        self.use_proxy = config.getboolean('settings','use_proxy')
        if self.use_proxy:
            user_pass = config.get('settings','user_pass')
        ...
        if logged:
            ...
            if response:
                ...
                if final:
                    ...
                    changeDate = config.getboolean('settings', 'change_date')
                    ...
                    if res:
                        ...
                        with open(f'{file_name}-notchanged.csv','a') as f:
                            f.write(f'{self.account}\n')
                    self.updateFile()
                    return True
                else:
    ...
    def changeDate(self):
        try:
            ...
            headers = {
                ...
                "accept-language": "en-US,en;q=0.9",
                # Add the cookie if necessary:
                "cookie": f"dslang=US-EN; site=USA; idclient=web; myacinfo={accinfo}; caw={caw}; aidsp={aidsp}; dat={dat}; caw-at={caw_at}; awat={awat}; itspod=39; pltvcid=undefined; aid={aid}"
            }
            payload = {
            "dayOfMonth": "01",
            "monthOfYear": "01",
            ...
            if '1990-01-01' in response.text:
                ...
                if '1990-01-01' in response.text:
                    print(f"{Fore.YELLOW}[{BLUE}{self.index}/{accounts_len}{RESET}]{Fore.LIGHTGREEN_EX} Success Change Date .. {RESET}")
                    self.birthday = '01/01/1990'
                    self.account = f'{self.email},{self.password},{self.birthday},{self.q1},{self.q2},{self.q3}'
                    with open('succes.csv','a') as f:
                        f.write(f'{self.saccount}\n')

                    return True
                else:
                    print(f"{Fore.YELLOW}[{BLUE}{self.index}/{accounts_len}{RESET}]{Fore.RED} Problem in Change Date .. {RESET}",response.json())
                    self.session.close()
                    try:
                        self.account = f'{self.email},{self.password},{self.birthday},{self.q1},{self.q2},{self.q3}'
                    except:
    ...
    def changeRegion(self):
            ...
            'Accept-Language': 'en-US,en;q=0.9',
            'Content-Type': 'application/json',
            'cookie':f"idclient=web; dslang=US-EN; site=USA; geo=IL; aid={self.aid}; dat={self.dat}; myacinfo={self.myacinfo}; awat={self.awat}; caw={self.caw}; caw-at={self.caw_at}; aidsp={self.aidsp}",
            'Referer': 'https://appleid.apple.com/',
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36',
            'X-Apple-I-FD-Client-Info': '{"U":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36","L":"en-US","Z":"GMT+03:00","V":"1.1","F":"sla44j1e3NlY5BNlY5BSs5uQ32SCVgecFW2A2p9ffSKk6Hb9LarUqUdHz16rgNNlejV9dY.Mel9SpDK1cDvkjmxMuijjNklY5BNleBBNlYCa1nkBMfs.7wM"}',
            'X-Apple-I-Request-Context': 'ca',
            ...
            target_region = None
            if config.getboolean('region', 'region_random'):
                regions = ['USA', 'GBR', 'DEU', 'HKG', 'IDN', 'MYS', 'THA', 'NLD', 'POL', 'EGY', 'ZAF', 'IRQ']
            ...
            region_data = {
                ...
                "IDN": {
                    "billingAddress": {
                        "line1": "Jl. Merdeka Selatan",
                        "city": "Jakarta",
                        "stateProvinceName": "JK",
                        "postalCode": "10110",
                        "countryCode": "IDN"
                    },
                    "phoneNumber": {
                ...
                "MYS": {
                    "billingAddress": {
                        "line1": "Menara Petronas",
                        "city": "Kuala Lumpur",
                        "stateProvinceName": "Kuala Lumpur",
                        "postalCode": "50088",
                        "countryCode": "MYS"
                    },
                    "phoneNumber": {
                ...
                "THA": {
                    "billingAddress": {
                        "line1": "30 Sukhumvit Road",
                        "city": "Bangkok",
                        "stateProvinceName": "Bangkok",
                        "postalCode": "10110",
                        "countryCode": "THA"
                    },
                    "phoneNumber": {
                ...
                "IRQ": {
                    "billingAddress": {
                        "line1": "121 Departure terminal road",
                        "city": "New Baghdad",
                        "stateProvinceName": "BG",
                        "postalCode": "10062",
                        "countryCode": "IRQ"
                    },
                    "phoneNumber": {
            ...
            if target_region not in region_data:
                print(f"{RED}[{self.index}] Invalid region code: {target_region}.{RESET}")
                return False

            region_info = region_data[target_region]
            payment_data = {
                "ownerName": {
                ...
                "phoneNumber": {
                    "areaCode": region_info["phoneNumber"]["areaCode"],
                    "number": region_info["phoneNumber"]["number"],
                    "countryCode": region_info["phoneNumber"]["countryCode"]
                },
                "billingAddress": {
                    "id": "1",
                    "line1": region_info["billingAddress"].get("line1", ""),
                    "line2": region_info["billingAddress"].get("line2", ""),
                    "line3": region_info["billingAddress"].get("line3", ""),
                    "suburb": region_info["billingAddress"].get("suburb", ""),
                    "county": region_info["billingAddress"].get("county", ""),
                    "city": region_info["billingAddress"].get("city", ""),
                    "stateProvinceName": region_info["billingAddress"].get("stateProvinceName", ""),
                    "postalCode": region_info["billingAddress"].get("postalCode", ""),
                    "countryCode": region_info["billingAddress"]["countryCode"]
                },
                "id": 1
            }
            ...
            if response.status_code == 200:
                shipping_data = {
                    ...
                    "countryCode": region_info["billingAddress"]["countryCode"],
                    "company": "",
                    "recipientFirstName": self.FNAME,
                    "recipientLastName": self.LNAME,
...
def main():
    ...
    try:
        ...
        ACCS.extend(all_list)
        ACCSUPDATE = ACCS.copy()
        accounts_len = len(ACCS)

        enteries_cnt = (accounts_len + browser_num - 1) // browser_num  
        
        cnt = 0
        with concurrent.futures.ThreadPoolExecutor(max_workers=browser_num) as executor:
            futures = []
            for i in range(enteries_cnt):
                for j in range(browser_num):
                    if cnt < accounts_len:
                        account = ACCS[cnt]
                        v = apple(cnt, account)
                        futures.append(executor.submit(v.start))
                        cnt += 1
